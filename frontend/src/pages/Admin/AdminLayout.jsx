...
export default function AdminLayout() {
  ...
  const { currentUser: user, logout, defaultImage } = useUserStore(); // Added defaultImage
          ...
          <Dropdown placement="bottom-end">
            ...
            <DropdownMenu aria-label="Profile Actions" variant="flat">
              <DropdownItem key="profile" className="h-14 gap-2">
                ...
                <p className="font-semibold">{user?.email ?? "admin@example.com"}</p>
              </DropdownItem>
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden w-full bg-white">
            <NavbarContent className="flex flex-col p-4 gap-2">
              ...
              <NavbarItem>
                <Link color="foreground" href="/admin/profile" className="w-full">
                  Profile
                </Link>
              </NavbarItem>
              <NavbarItem>
                <Link color="danger" href="/logout" className="w-full" onClick={handleLogout}>
                  Logout
                </Link>
              </NavbarItem>
            </NavbarContent>
